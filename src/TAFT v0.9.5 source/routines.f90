   ...
   SUBROUTINE FAST(b, N, Ne, k, p0, distobs, NoS, S, t, treal, splan, sim, counter, c, CONTIN01, FSTobs, FSTT, CountFST)
	   
	   !This subprogram performs a number (sim) of simulations with fixed initial allele frequencies (p0). 
       ...
       REAL, INTENT(IN), DIMENSION(NoS):: treal     !Same as t for semi-continuous generations
       INTEGER, INTENT(IN):: splan			        !Switch para usar el plan de muestreo I o II
       REAL(8), INTENT(IN):: sim                    !Number of simulations
       REAL(8), INTENT(INOUT) :: counter	        !Counter of positive cases
	   REAL(8), INTENT(INOUT) :: c                  !Counter of simulations
	   LOGICAL, INTENT(IN):: CONTIN01               !True if continuous generations
	   REAL(8), INTENT(IN):: FSTobs					!Onserved value of Fst
	   REAL, INTENT(INOUT):: FSTT 				    !Temporal Fst 
	   INTEGER, INTENT(INOUT):: CountFST			!Counter of positive temporal Fst's (significance)
	
       !Internals
       INTEGER:: N_hy, n_hyp                            ! Variables used by hypergeometric sampler
	   ...
	   REAL, ALLOCATABLE, DIMENSION (:):: qr            ! Relative allele frequencies at effective population   
	   REAL:: Distsim, FSTTlocal                        ! Distance among simulated samples; FSTT of the current simulation
	   INTEGER:: j, i, l, m, w, Total                   ! Counters
	   REAL:: FD                                        ! Remaining fraction of the number of generations   
	   !LOGICAL ::first=.true. 
	   LOGICAL ::Switch                        
	   REAL(8):: HT, HS, Hx, Hy 
	   !-----------------------------------------------------------------------------------------------
	   ...
	   DO
           ...
           !--------------------------------
	       !LLAMAMOS AL GENERADOR DE HIPERGEOMÉTRICAS PARA LA MUESTRA CERO		   
1500	   CALL multhyper(k,p0,S(1),xy(1,:))
	       
	       DO w = 1,  NoS-1, 1
	           !MODIFICACIONES PARA EL PLAN 1 (ANTES DE LA REPRODUCCIÓN):
	           !LOS ORGANISMOS NO SE REEMPLAZAN PARA OBTENER A LA POBLACIÓN EFECTIVA
	           IF(splan==2) THEN
	              DO l=1, k, 1
	           ...
	           ENDIF
	           !CICLO PARA GENERAR TODAS LAS GENERACIONES INTERMEDIAS ANTES DE OBTENER LA MUESTRA
	           !DE LA GENERACIÓN T.  
               DO m=t(w)+1, t(w+1), 1		!m=t+1!
	              IF (m==t(w)+1) THEN
                     !LLAMAMOS AL GENERADOR DE HIPERGEOMÉTRICAS PARA LA POBLACIÓN EFECTIVA
					 CALL multhyper(k,p(m,:),Ne(m),q(m,:)) 

			      ENDIF
		          !LLAMAMOS AL GENERADOR DE BINOMIALES PARA OBTENER P DE LA SIGUIENTE GENERACIÓN 
	              IF (m<t(w+1)) THEN
		             DO i=1, k-1, 1
                  ...
                  ENDIF
               ENDDO   
               !SI LAS GENERACIONES SON SEMI-CONTINUAS Y EL NO. DE GENERACIONES NO ES ENTERO:
	           IF ((CONTIN01).AND.(treal(w+1)-REAL(t(w+1))>0.0)) THEN
	               FD = treal(w+1)-REAL(t(w+1))
		           !PRIMERO GENERAMOS A LA POBLACIÓN EFECTIVA QUE ORIGINARA A LA FRACCIÓN DE LA GENERACIÓN
		           !SIGUIENTE QUE EXISTE AL MOMENTO DE TOMAR LA MUESTRA
				   CALL multhyper(k,p(t(w+1)+1,:),Ne(t(w+1)+1),q(t(w+1)+1,:)) 				   

		           !AHORA GENERAMOS A LOS GENES PRESENTES DE LA GENERACIÓN SIGUIENTE (BINOMIAL)
		           DO i=1, k-1, 1
		              qr(i)=REAL(q(t(w+1)+1,i))/REAL(Ne(t(w+1)+1))
		           ENDDO
		           CALL genmul(NINT(REAL(N(t(w+1)+2)*FD)), qr, k, p(t(w+1)+2,:))
                   !	LA FRECUENCIA SIMULADA EN LA POBLACIÓN ACTUAL ES UNA MEZCLA DE LA DE 
		           ! LA GENERACIÓN ACTUAL Y LA FRACCIÓN DE LA GENERACIÓN SIGUIENTE  
		           Total = 0
		           DO i=1, k, 1
		              p(t(w+1)+2,i)= p(t(w+1)+2,i) + NINT(REAL(p(t(w+1)+1,i))*(1-FD))  !WE BECAME GENERATION T(W+1)+2
			          Total = p(t(w+1)+2,i) + Total	  							   !A MIXTURE OF THE SAMPLING GENERATION (T(W+1)+1)
				   ENDDO															   !WITH ITSELF IN THE PROPORTION GIVEN BY "FD"
		           !LLAMAMOS AL GENERADOR DE HIPERGEOMÉTRICAS PARA LA MUESTRA T				   
				   CALL multhyper(k,p(t(w+1)+2,:),S(w+1),xy(w+1,:)) 				   

			   ELSE ! IF THERE IS NO CONTINUOUS GENERATION AT THIS SAMPLING
	   	            ! NOTE: T(w+1)+2 IS ONE GENERATION AFTER THE SAMPLED ONE 
	               !LLAMAMOS AL GENERADOR DE HIPERGEOMÉTRICAS PARA LA MUESTRA T				   
		   ...
		   !SWITCH IS USED TO SKIP SIMULATIONS WHERE AT LEAST ONE ALLELE IS ABSENT FROM ALL SAMPLES 
		   !DO i=1, k, 1
		   !   Switch = .FALSE.
		   !   DO l = 1, NoS, 1
		   !      IF(xy(l,i)/=0) THEN
...
!250        IF (Switch) THEN
		      j=0
	          C=C+1  
	          Distsim=0.0      !WE TAKE THE DISTANCE AMONG FREQUENCIES OF CONSECUTIVE SAMPLES 
		      DO l = 1, NoS-1, 1 
	             DO i=1, k-1, 1
...
500    FORMAT(' ',F10.4,'%')
   ...
   SUBROUTINE Dirichlet_param(k,NoS,xy,le,Ne,S,t,D)
         ...
         REAL(16), DIMENSION(k), INTENT(OUT):: D         !Vector of parameters adjusted 
   ...
   SUBROUTINE Frec_Estim (k, NoS, b, xyobs, S, t, N, Ne, splan, FrecEst)
		...
		INTEGER, INTENT(IN):: splan
		REAL, INTENT(OUT), DIMENSION(k):: FrecEst
		!INTERNAL VARIABLES
		INTEGER:: i, j, l, a
		REAL:: phiN, M, SigFrec 				 ! Suma de N y de Ne en caso de N/Ne variable en el tiempo
        REAL, ALLOCATABLE :: Sigma(:,:), SigmaInv(:,:)  
	    REAL, ALLOCATABLE :: Frec(:,:), SigVector(:), Vector(:)  
	    REAL::det(2)	
	    INTEGER:: info
		!----------------------------------------------------------------------------------------------------
		! I   N   I   C   I   O
...
!
      REAL, INTENT(IN):: X, F, THETA
      REAL, INTENT(OUT):: Y
      INTEGER, INTENT(OUT):: IFAULT
!
      INTEGER ITRMAX
      LOGICAL FLAG
      DOUBLE PRECISION:: ERRMAX, ZERO, ONE, TWO, LAM, N, U, V, X2, F2, T, TERM, BOUND !, ALNGAM 
!
      !EXTERNAL ALNGAM
!
