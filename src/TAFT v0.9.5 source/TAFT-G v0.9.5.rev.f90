...
!****************************************************************************
   ...
   program TAFTGv095
       ...
       if (COMMAND_ARGUMENT_COUNT().NE.2) then
          write(*,*)'Error: input and output file names needed.'
          STOP
       endif
       ...
       P_Chi=0
       !++
       OPEN(UNIT=1, FILE=ifile, STATUS='OLD', ACTION='READ', IOSTAT=ierror) ! Opening the file	    
       ...
       !**********************************************************************************************
       if (seed1.LT.0) then
          call random_number(useed)
          seed1 = FLOOR(huge(seed1)*useed)
          write(*,*) 'Random seed randomlly init as ', seed1
       endif
       ...
       do l=1,nloci,1
         ...
         inquire(FILE=ofile, EXIST=found) 
         if (found) then
              if (l==1) then   !We replace (if found) only the first time; 
                    OPEN(UNIT=1, FILE=ofile, STATUS='REPLACE', ACTION='WRITE', IOSTAT=ierror) 
              else
                    OPEN(UNIT=1, FILE=ofile, STATUS='OLD', ACTION='WRITE', ACCESS='APPEND', IOSTAT=ierror)  
              endif  
         else
              OPEN(UNIT=1, FILE=ofile, STATUS='NEW', ACTION='WRITE', IOSTAT=ierror)    
         endif 
