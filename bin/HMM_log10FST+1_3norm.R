...
# base<-paste(elements[[1]][-length(elements[[1]])],collapse=".")
# rm(elements)
base<-paste(trimws(args[1], which='right', whitespace='/'), '_results', '/', sep='')
# base<-paste(args[1], '_results', '/', sep='')
dir.create(base)
...
fst<-data$x
fst<-fst[!is.na(fst)]
lfst<-log10(fst[fst>quantile(fst,0.001)]+1)
# fst<-fst[fst>=1e-5]
# lfst<-log(fst/(1-fst))
...
hm.split = min(0.15, quantile(lfst, 0.99)) # fcut of temporal (12 genotypes, 0.05)
ml.split = ifelse(min(lfst)<0,-min(lfst),hm.split/2)
stopifnot(hm.split > ml.split)
lfst.means = c(mean(lfst[lfst < ml.split]), mean(lfst[ml.split <= lfst & lfst < hm.split]), mean(lfst[hm.split <= lfst]))
lfst.sds   = c(  sd(lfst[lfst < ml.split]),   sd(lfst[ml.split <= lfst & lfst < hm.split]),   sd(lfst[hm.split <= lfst]))

# mean.lfst<-mean(lfst)
# sd.lfst<-sd(lfst)
# lfst.means = c(mean.lfst*0.9, mean.lfst, mean.lfst*1.1)
# lfst.sds   = rep(sd.lfst/3,3)
...
parout<-foreach(i=1:as.integer(args[3]),.packages="HiddenMarkov",.combine='c') %dopar% {
  ...
  means<-(lfst.means*sort(rnorm(3,mean=1,sd=0.2)))
  sdevs<-(lfst.sds*rnorm(3,mean=1,sd=0.2))
...
save.image(file = paste(base,"envs.RData",sep=""))
...
hmmpar = hmmpar[!(hmmpar$m1 > hmmpar$m2 | hmmpar$m2 > hmmpar$m3),]
...
plot.likelihood <- function(base,hmmpar){
  tryCatch(
    {
    ...
    warning=function(cond){
      message('Warning caught:')
      message(cond)
    }
  )
}
plot.likelihood(base,hmmpar)
...
plot.distribution <- function(base,lfst,bwhmm){
  tryCatch(
    {
...
plot.distribution(base,lfst,bwhmm)
